...
function Test-VPC {
    ...
    if ([string]::IsNullOrEmpty($VpcId)) {
        Write-Host "[ERROR] VPC ID not found in Terraform outputs" -ForegroundColor $Red
        return $false
    }
    ...
    if ($LASTEXITCODE -eq 0 -and $result -eq "available") {
        Write-Host "[OK] VPC exists and is available" -ForegroundColor $Green
        return $true
    } else {
        Write-Host "[ERROR] VPC not found or not available" -ForegroundColor $Red
        return $false
    }
}
...
function Test-Subnets {
    ...
    if ([string]::IsNullOrEmpty($PublicSubnetId) -or [string]::IsNullOrEmpty($PrivateSubnetId)) {
        Write-Host "[ERROR] Subnet IDs not found in Terraform outputs" -ForegroundColor $Red
        return $false
    }
    ...
    if ($LASTEXITCODE -eq 0 -and $publicState -eq "available") {
        Write-Host "[OK] Public subnet is available" -ForegroundColor $Green
    } else {
        Write-Host "[ERROR] Public subnet not available" -ForegroundColor $Red
        return $false
    }
    ...
    if ($LASTEXITCODE -eq 0 -and $privateState -eq "available") {
        Write-Host "[OK] Private subnet is available" -ForegroundColor $Green
        return $true
    } else {
        Write-Host "[ERROR] Private subnet not available" -ForegroundColor $Red
        return $false
    }
}
...
function Test-InternetGateway {
    ...
    if ([string]::IsNullOrEmpty($IgwId)) {
        Write-Host "[ERROR] Internet Gateway ID not found" -ForegroundColor $Red
        return $false
    }
    ...
    if ($LASTEXITCODE -eq 0 -and $igwState -eq "available") {
        Write-Host "[OK] Internet Gateway is available" -ForegroundColor $Green
        return $true
    } else {
        Write-Host "[ERROR] Internet Gateway not available" -ForegroundColor $Red
        return $false
    }
}
...
function Test-NATGateway {
    ...
    if ([string]::IsNullOrEmpty($NatId)) {
        Write-Host "[ERROR] NAT Gateway ID not found" -ForegroundColor $Red
        return $false
    }
    ...
    if ($LASTEXITCODE -eq 0 -and $natState -eq "available") {
        Write-Host "[OK] NAT Gateway is available" -ForegroundColor $Green
        return $true
    } else {
        Write-Host "[ERROR] NAT Gateway not available (State: $natState)" -ForegroundColor $Red
        return $false
    }
}
...
function Test-EC2Instances {
    ...
    if ([string]::IsNullOrEmpty($PublicIp) -or [string]::IsNullOrEmpty($PrivateIp)) {
        Write-Host "[ERROR] EC2 IPs not found in Terraform outputs" -ForegroundColor $Red
        return $false
    }
    ...
    try {
        ...
        if ($response.StatusCode -eq 200) {
            Write-Host "[OK] Public EC2 HTTP service is reachable" -ForegroundColor $Green
            return $true
        }
    }
    catch {
        Write-Host "[WARNING] Public EC2 HTTP service not reachable (may still be starting)" -ForegroundColor $Yellow
...
function Test-RouteTables {
    ...
    if ([string]::IsNullOrEmpty($VpcId)) {
        Write-Host "[ERROR] VPC ID not found for route table testing" -ForegroundColor $Red
        return $false
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "[ERROR] Failed to retrieve route tables" -ForegroundColor $Red
        return $false
    }
    ...
    $routeTables -split "`n" | ForEach-Object {
        ...
        if ($parts.Length -ge 2) {
            ...
            if ($rtName -like "*Public*") {
                $publicRTFound = $true
                Write-Host "[OK] Public Route Table found: $($parts[0])" -ForegroundColor $Green
            }
            elseif ($rtName -like "*Private*") {
                $privateRTFound = $true
                Write-Host "[OK] Private Route Table found: $($parts[0])" -ForegroundColor $Green
            }
        }
    }
    
    if ($publicRTFound -and $privateRTFound) {
        Write-Host "[OK] Both Public and Private Route Tables exist" -ForegroundColor $Green
        return $true
    } else {
        Write-Host "[ERROR] Missing route tables - Public: $publicRTFound, Private: $privateRTFound" -ForegroundColor $Red
        return $false
    }
}
...
function Test-DefaultSecurityGroup {
    ...
    if ([string]::IsNullOrEmpty($VpcId)) {
        Write-Host "[ERROR] VPC ID not found for default SG testing" -ForegroundColor $Red
        return $false
    }
    ...
    if ($LASTEXITCODE -eq 0 -and $defaultSG -ne "None" -and ![string]::IsNullOrEmpty($defaultSG)) {
        Write-Host "[OK] Default Security Group exists: $defaultSG" -ForegroundColor $Green
        return $true
    } else {
        Write-Host "[ERROR] Default Security Group not found" -ForegroundColor $Red
        return $false
    }
}
...
function Test-SecurityGroupRules {
    ...
    if ([string]::IsNullOrEmpty($PublicSgId) -or [string]::IsNullOrEmpty($PrivateSgId)) {
        Write-Host "[ERROR] Security Group IDs not found" -ForegroundColor $Red
        return $false
    }
    ...
    if ($LASTEXITCODE -eq 0 -and ![string]::IsNullOrEmpty($publicRules)) {
        Write-Host "[OK] Public Security Group has SSH rule (port 22)" -ForegroundColor $Green
    } else {
        Write-Host "[ERROR] Public Security Group missing SSH rule" -ForegroundColor $Red
        return $false
    }
    ...
    if ($LASTEXITCODE -eq 0 -and ![string]::IsNullOrEmpty($privateRules)) {
        Write-Host "[OK] Private Security Group allows access from Public SG" -ForegroundColor $Green
        return $true
    } else {
        Write-Host "[ERROR] Private Security Group missing rules from Public SG" -ForegroundColor $Red
        return $false
    }
}
...
function Main {
    ...
    if (-not (Test-Path "../terraform/terraform.tfstate")) {
        Write-Host "[ERROR] Terraform state file not found. Please run 'terraform apply' first." -ForegroundColor $Red
        exit 1
    }
    ...
    if ($FailedTests -eq 0) {
        ...
        Write-Host "[ERROR] $FailedTests test(s) failed" -ForegroundColor $Red
